...
def calibrate_gauge(gauge_number, file_type):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 50,
                               np.array([]), 100, 50, int(height * 0.1), int(height * 1))
    ...
    for i in range(0, interval):
        for j in range(0, 2):
            if j % 2 == 0:
                p1[i][j] = x + 0.8 * r * np.cos(separation * i * 3.14 / 180)  # point for lines
            else:
                p1[i][j] = y + 0.8 * r * np.sin(separation * i * 3.14 / 180)
    text_offset_x = 10
    text_offset_y = 5
    for i in range(0, interval):
        for j in range(0, 2):
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    # th, dst2 = cv2.threshold(gray2, thresh, maxValue, cv2.THRESH_BINARY_INV)
    dst2 = cv2.adaptiveThreshold(gray2, maxValue, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)
    ...
    # diff2LowerBound and diff2UpperBound determine how close the other point of the line
    # should be to the outside of the gauge
    diff2LowerBound = 0.3  # 0.5
    ...
    for i in range(0, len(final_line_list)):
        ...
        cv2.putText(img2, '%s' % (int(i)), (int(final_line_list[i][0]), int(final_line_list[i][1])),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
        cv2.imwrite('gauge-%s-lines-test-2.%s' % (gauge_number, file_type), img2)
